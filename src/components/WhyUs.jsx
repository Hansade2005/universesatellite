...
export default function WhyUs() {
  ...
  const reasons = t('why');
  return (
    <section id="why" style={{ padding: '7rem 0', position: 'relative' }}>
      ...
      <div className="container" ref={ref} style={{ position: 'relative', zIndex: 2 }}>
          ...
          {reasons.map((r, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.15 * i }}
                style={{ padding: '2rem', borderRadius: '20px', background: i === 0 ? 'linear-gradient(135deg, var(--red), var(--red-dark))' : 'rgba(255,255,255,0.02)', border: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.06)', transition: 'all 0.3s ease' }}
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.6 }}
          style={{ position: 'relative', padding: '3.5rem', borderRadius: '24px', overflow: 'hidden', background: 'linear-gradient(135deg, rgba(227,6,19,0.1), rgba(227,6,19,0.03))', border: '1px solid rgba(227,6,19,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem' }}>
